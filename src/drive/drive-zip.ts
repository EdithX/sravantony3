        ...
        driveAuth.call(async (err, auth) => {
            ...
            await drive.files.get({ fileId: fileId, fields: 'id, name, mimeType, size', supportsAllDrives: true }).then(async meta => {
                ...
                if (meta.data.mimeType === FOLDER_TYPE) {
                    ...
                    const duplicate = await isDuplicateMirror(originalFileName + '.zip');
                    if (duplicate) {
                    ...
                    let res = await downloadAllFiles(meta.data, drive, folderPath, bot, zippingMsg, message);
                    if (res.message && res.message.includes('found')) {
                        ...
                        zip.archive(realFilePath, destName, (ziperr: string, size: number) => {
                            if (ziperr) {
                                ...
                                driveUploadFile(realFilePath + '.zip', dlDetails, (uperr, url, isFolder, indexLink) => {
...
var lastMessage = '';
export function updateStatus(dlDetails: DlVars, totalsize: number, message: string, bot: TelegramBot, zippingMsg: TelegramBot.Message): void {
    let sm = getStatus(dlDetails, totalsize);
    message += `\n\n` + sm.message;
    if (lastMessage !== message) {
        msgTools.editMessage(bot, zippingMsg, message).catch(e => {
...
async function downloadAllFiles(meta: drive_v3.Schema$File, drive: drive_v3.Drive, folderPath: string, bot: TelegramBot, zippingMsg: TelegramBot.Message, message: string) {
    let errMsg: boolean;
    let rmessage: string;
